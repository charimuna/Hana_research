...
# CSV output
# ------------------------------------------
or_table.to_csv(OUTPUT_CSV, index=False, encoding="utf-8-sig")
...
with open(OUTPUT_CSV, "a", encoding="utf-8-sig") as f:
    ...
    f.write(f"Events,{int((df['unex_death'] == 1).sum())}\n")
    f.write(f"Non-events,{int((df['unex_death'] == 0).sum())}\n")
    f.write(f"Hosmer-Lemeshow Chi2,{HL}\n")
    f.write(f"Hosmer-Lemeshow df,{df_hl}\n")
    f.write(f"Hosmer-Lemeshow P,{p_hl}\n")
    f.write(f"AUC,{auc}\n")

print(f"\nCSV output completed: {OUTPUT_CSV}")
